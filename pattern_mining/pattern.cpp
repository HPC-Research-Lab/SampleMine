...
namespace euler::pattern_mining {
  ...
  static void
    report_aut(void* param, const unsigned int N, const unsigned int* perm)
  {
    assert(param);
    assert(N > 0);
    assert(perm);
    auto orbits = (std::vector<std::vector<unsigned>>*)param;
  ...
  std::tuple<std::string, std::vector<std::vector<unsigned>>, std::vector<unsigned>> Pattern::canonical_form() const {
    if (!use_vertex_label) {
      std::cerr << "no label" << std::endl;
      exit(-1);
    }
