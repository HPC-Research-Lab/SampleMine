...
bool nested_for_loop(
    ...
    std::map<std::string, std::tuple<int, std::shared_ptr<Pattern>, std::vector<int>>> &canonical_patterns,
...
#pragma omp parallel for num_threads(_Nthreads)
    ...
    for (int i = 0; i < vertex_set[tid][level].size(); i++) {
      ...
      if (has_label) {
        ...
        const int *perm = cp.second.data();
#pragma omp critical
        {
          if (canonical_patterns.find(cp.first) == canonical_patterns.end()) {
            canonical_patterns[cp.first] = std::make_tuple(canonical_patterns.size(), ptt, cp.second);
          }
          if (store_data) {
            v[tid][0] = std::get<0>(canonical_patterns[cp.first]);
            data->merge(std::to_string(v[tid][0]), v[tid].data(), v[tid].size() * sizeof(int), store_data, mni, perm);
          }
        }
      } else {
        if (store_data) {
...
SGList match(const graph::Graph &g, const PatList &patterns, bool store_data, bool edge_induced, bool has_label, size_t mni, double sampling_threshold) {
  ...
  for (auto &p : patterns) {
    ...
    // TODO: need to fix for labeled matching

    /*
    if (has_label && store_data) {
      auto d1 = data->getbuf(data->keys[key], data->ncols);


      // change the index in each subgraph to a quick pattern index
      for (auto it_d1 = d1.begin(); it_d1 != d1.end(); ++it_d1) {
        auto ptt = std::make_shared<Pattern>(*pat);
        ptt->enable_label();

        for (int k = 1; k < data->ncols; k++) {
          int l = g.get_vertex_label((*it_d1)[k]);
          ptt->add_label(l);
        }
        ...
        **it_d1 = std::get<0>(canonical_patterns[cp.first]);
        data_with_labels->merge(std::to_string(**it_d1), *it_d1, s1 * sizeof(int), store_data, mni, perm);
      }

      assert(data_with_labels->size() == patterns_with_labels.size());
    }
    */
  }

  if (has_label) {
    for (auto &cp : canonical_patterns) {
      patterns_with_labels.insert({std::get<0>(cp.second), std::get<1>(cp.second)});
    }
    if (store_data) assert(data->size() == canonical_patterns.size());
    return SGList(data, patterns_with_labels, 0, 0, 0);
  } else {
