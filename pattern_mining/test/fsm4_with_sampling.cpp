...
int main(int argc, char *argv[])
{
  ...
  for (int oiter = 0; oiter < 8; oiter++)
  {
    util::Timer t;
    t.start();
    auto d_res = join<true, true, true, 2, 4, 3>(g, H, sgls, false, clustered, {st1, st2}, mni_threshold);
    t.stop();
    ...
    if (acc_res.sgl == nullptr) acc_res = d_res;
    else acc_res.combine(d_res, true, false);

    cout << "acc number of frequent patterns: " << acc_res.sgl->size() << endl;
  }
