...
//    double scaled_st2 = scale_sampling_param(d2, st2);
  ...
  Sampler *sm2;
  if (st2 > 0)
      sm2 = new BudgetSampler(subgraph_hist, { st2_scaled * st2_scaled, st2_scaled * st2_scaled, st2_scaled * st2_scaled });
  ...
  util::Timer t;
  t.start();
  auto [d_res, ess] = join<true, true, true, true, 3, 4, 4, 4>(g, H, sgls, false, *sm2, sup, false, false);
  t.stop();
