...
int main(int argc, char* argv[]) {
  ...
  cout << "start join for pat5: " << endl;
  vector<SGList> sgls2 = { d2, d2, d2, d2};

  Sampler *sm1;
  if (st1 != 1)
    sm1 = new ProportionalSampler2({ st1, st1, st1, st1 });
  ...
  auto [d, ess] = join<true, true, false, false, 4, 3, 3, 3, 3>(g, H2, sgls2, true, *sm1, -1, true);

  match_time.stop();

  cout << "join for pat5 time: " << match_time.get() << " sec" << endl << flush;
