...
int main(int argc, char* argv[]) {
  ...
  vector<SGList> sgls = { d2, d2, d2, d2, d2 };
  ...
  for (int i = 0; i < sampling_rounds; i++) {
    ...
    util::Timer t;
    t.start();
    auto [d_res, ess] = join<true, true, true, false, 5, 3, 3, 3, 3, 3>(g, H, sgls, false, sm, { st_scaled, st_scaled, st_scaled, st_scaled, st_scaled }, subgraph_hist, sup);
    t.stop();
