...
int main(int argc, char* argv[]) {
  ...
  auto H2 = build_tables(sgls2);
    ...
    Sampler *sm_tmp = new ProportionalSampler2({ 8, 8 });
  ...
  Sampler *sm2;
  if (st2 > 0)
    sm2 = new ProportionalSampler2({ st2, 256, 256 });
  ...
  util::Timer t;
  t.start();
  auto [d_res, ess] = join<false, true, false, false, 3, 4, 3, 3>(g, H, sgls, false, *sm2, -1, false, st2 > 0, false, join_dummy1, query);
  t.stop();
