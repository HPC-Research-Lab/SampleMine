...
int main(int argc, char* argv[]) {
  ...
  auto pat2 = pattern_mining::PatListing::make_pattern(
   ...
   auto pat5 = pattern_mining::PatListing::make_pattern(
    pattern_mining::PatListing().pattern_listing(6));
  ...
  Sampler* sm2;
  if (st2 > 0)
    sm2 = new ProportionalSampler2({ st2*st2, st2*st2,  st2 });
  else sm2 = &default_sampler;

  util::Timer t;
  t.start();
  auto [d_res, ess] = join<true, true, false, false, 3, 4, 4, 3>(g, H, sgls, false, *sm2, -1, false, st2 > 0);
  t.stop();
