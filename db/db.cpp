...
template <class key_type, class value_type>
MyKV<key_type, value_type>::MyKV(int nc) : ncols(nc), data_size(0), mni_buf_size(0) {
...
void MyKV<key_type, value_type>::merge(const key_type &k, const void *a, size_t len,
  ...
  if (!first) {
    if (mni > 0) {
      ...
      /*if (mni_buf_size >= BUF_SIZE) {
        std::cerr << "mni write file" << std::endl;
        std::cerr << keys.size() << " " << buf.size() << std::endl;
        for (int i = 0; i < nfiles; i++) {
          for (int j = 0; j < ncols - 1; j++) {
            std::ofstream fd((DBPath + "/mni_" + std::to_string(i) + "_" + std::to_string(j) + ".dat").c_str(),
                             std::ios::out | std::ios::app);
            if (!fd) std::cerr << "Error opening mni file" << std::endl;
            for (auto v : distinct_vertices[i][j]) {
              fd << v << std::endl;
            }
            fd.close();
            distinct_vertices[i][j].clear();
          }
          //distinct_vertices[i].clear();
          //std::vector<int>().swap(buf[i]);
        }
        std::cerr << "mni backup done" << std::endl;
        mni_buf_size = 0;
      }*/
      if (!mni_met[file_id]) {
